...
**Always find and fix the root cause of LLM failures. Never patch symptoms by adding more prompt instructions.**

Before appending "IMPORTANT:" caveats or extra constraints to a prompt, ask *why* the model is failing:
- Is the context injection pipeline sending malformed or irrelevant data?
- Is the prompt structure itself ambiguous or contradictory?
- Is the tool schema missing required fields or examples?
- Is the system prompt conflicting with few-shot examples?
- Is a retrieval step returning garbage that poisons the context window?

Fix the originating defect (pipeline, schema, retrieval, prompt structure) rather than layering compensating instructions on top of a broken foundation. Prompt patches that add "you MUST..." or "NEVER..." without fixing the underlying issue are fragile and will break on novel inputs.
