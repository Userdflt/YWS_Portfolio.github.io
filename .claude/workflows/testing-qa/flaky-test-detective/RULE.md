...
**Always fix the root cause of flakiness, not just the test.**

A flaky test is a symptom. The root cause is usually in the application code, test infrastructure, or environment — not in the test assertion itself. Before applying any test-level fix (adding `waitFor`, replacing `getBy` with `findBy`, adding cleanup hooks), ask:
- Is there a race condition or timing bug in the component/service being tested?
- Is shared mutable state leaking between tests because the application doesn't isolate properly?
- Is the code under test non-deterministic when it should be deterministic?
- Is the test environment misconfigured?

If the answer is yes, fix the application code or infrastructure first. Test-level workarounds (extra waits, retries, cleanup hacks) are patches that hide the real defect and will cause problems elsewhere. Only apply test-level fixes when the application behavior is correct and the flakiness is purely a test-authoring issue.
